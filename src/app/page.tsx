...
async function fetchRepoStats(): Promise<RepoStats> {
  ...
  try {
    const [repoRes, contribRes, gfiRes, prsRes] = await Promise.all([
      fetch("https://api.github.com/repos/omen18/codestride", OPTS(3600)),
      fetch("https://api.github.com/repos/omen18/codestride/contributors?per_page=100", OPTS(3600)),
      fetch("https://api.github.com/repos/omen18/codestride/issues?labels=good+first+issue&state=open&per_page=100", OPTS(1800)),
      fetch("https://api.github.com/search/issues?q=repo:omen18/codestride+type:pr+is:merged&per_page=1", OPTS(3600)),
    ]);
