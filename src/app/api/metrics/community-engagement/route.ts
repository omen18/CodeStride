...
export interface CodeStrideBadge {
  id: string;
  title: string;
  description: string;
  emoji: string;
...
export async function GET(req: NextRequest) {
  ...
  const key = metricsCacheKey(user.id, "codestride-badges" as any, {});
  ...
  const badges = await withMetricsCache(
    ...
    async () => {
      ...
      if (newlyEarned.length > 0) {
        await supabaseAdmin.from("codestride_badges").upsert(
