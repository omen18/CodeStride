...
export default function FriendComparePage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("codestride:show-commit-activity", handleShowCommitActivity as EventListener);
    window.addEventListener("codestride:clear-compare-user", handleClearCommitActivity);
    return () => {
      window.removeEventListener("codestride:show-commit-activity", handleShowCommitActivity as EventListener);
      window.removeEventListener("codestride:clear-compare-user", handleClearCommitActivity);
    };
  }, []);
    ...
    if (showCommitActivity && compareUsername) {
      ...
      const timer = setTimeout(() => {
        window.dispatchEvent(
          new CustomEvent("codestride:compare-user", {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const persistedFriend = localStorage.getItem("codestride:compare_username");
        if (persistedFriend) {
