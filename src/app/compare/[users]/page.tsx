...
export async function generateMetadata({
  ...
  if (!parsed) {
    return {
      ...
      description: "Compare public CodeStride profile stats.",
    };
  }

  const [left, right] = parsed;
  return {
    title: `${left} vs ${right} - CodeStride Compare`,
    description: `Side-by-side public CodeStride stats comparison for ${left} and ${right}.`,
  };
}
        ...
        <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-[var(--muted-foreground)]">
              Shareable comparison built only from publicly visible CodeStride stats.
            </p>
          </div>
          <Link
...
function CompareUnavailable({
  title,
  message = "Use /compare/user1-vs-user2 with two public CodeStride profiles.",
}: {
  title: string;
  message?: string;
}) {
