...
function addFooter(doc: jsPDF, generatedAt: string) {
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    doc.text("Generated by CodeStride", 14, pageHeight - 10);
...
export default function ExportButton() {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text("Generated from CodeStride", marginX, 25);
      doc.setFontSize(9);
      ...
      doc.save(`codestride-export-${reportName || "metrics"}-${new Date().toISOString().slice(0, 10)}.pdf`);
  ...
  const exportJSON = async () => {
    ...
    try {
      ...
      downloadFile(
        ...
        `codestride-export-${reportName || "metrics"}-${new Date().toISOString().slice(0, 10)}.json`,
        "application/json"
      );
    } finally {
