...
export default function ContributionGraph() {
  ...
  const [days, setDays] = useState<number>(() => {
    if (typeof window !== "undefined") {
      try {
        const stored = localStorage.getItem("codestride:contribution-range");
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (stored === "7" || stored === "30" || stored === "90" || stored === "365") {
          ...
          localStorage.setItem("codestride:contribution-range", "30");
          setDays(30);
        }
      } catch (e) {
        setDays(30);
  ...
  const handleRangeChange = (newDays: number) => {
    ...
    if (typeof window !== "undefined") {
      try {
        localStorage.setItem("codestride:contribution-range", String(newDays));
      } catch (e) {}
    }
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener("codestride:compare-user", onCompareUser as EventListener);
    window.addEventListener("codestride:clear-compare-user", onClearCompareUser);

    return () => {
      window.removeEventListener("codestride:compare-user", onCompareUser as EventListener);
      window.removeEventListener("codestride:clear-compare-user", onClearCompareUser);
    };
  }, []);
  ...
  const handleClearCompare = () => {
    window.dispatchEvent(new Event("codestride:clear-compare-user"));
    setCompareMode(false);
    setCompareUser(null);
    setFriendData([]);
    setCompareError(null);
