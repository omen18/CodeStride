...
export default function ContributionHeatmap({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const stored = localStorage.getItem("codestride:heatmap-range");
        if (stored === "30" || stored === "90" || stored === "180" || stored === "365") {
          setSelectedDays(Number(stored));
        } else {
          localStorage.setItem("codestride:heatmap-range", String(days));
        }
      } catch (e) {
  ...
  const handleRangeChange = (newDays: number) => {
    ...
    if (typeof window !== "undefined") {
      try {
        localStorage.setItem("codestride:heatmap-range", String(newDays));
      } catch (e) {}
    }
  };
