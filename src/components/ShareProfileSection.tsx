...
export default function ShareProfileSection({
  ...
  const shareText = `Check out my coding stats on CodeStride! 🔥 ${streak}-day streak`;
  ...
  const handleNativeShare = async () => {
    ...
    try {
      await navigator.share({
        title: `${username}'s CodeStride Profile`,
        text: shareText,
        url: profileUrl,
      });
    } catch (error) {
  ...
  const downloadCard = async () => {
    ...
    try {
      ...
      link.download = `${username}-codestride-card.png`;
      link.href = dataUrl;
      link.click();
    } catch {
