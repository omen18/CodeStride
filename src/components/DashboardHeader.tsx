...
export function DashboardSyncProvider({ children }: { children: ReactNode }) {
  const [lastSynced, setLastSynced] = useState<Date | null>(() => {
    const stored = localStorage.getItem("codestride-last-synced");
  ...
  useEffect(() => {
    ...
    window.addEventListener("codestride:sync", handleSync);
    return () => {
      window.removeEventListener("codestride:sync", handleSync);
    };
  }, []);
        ...
        const doFetch = async () => {
          try {
            ...
            if (response.ok) {
              ...
              localStorage.setItem("codestride-last-synced", nowTime.toISOString());
            }
            return response;
          } catch (error) {
            throw error;
