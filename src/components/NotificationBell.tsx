...
export default function NotificationBell() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("codestride:unread-notification-count");
      if (stored !== null) {
    ...
    window.addEventListener("codestride:notifications", handleNotifications);

    return () =>
      window.removeEventListener("codestride:notifications", handleNotifications);
  }, [refetch]);
  ...
  const handleOpen = useCallback(async () => {
    setOpen((prev) => {
      ...
      if (!prev && unreadCount > 0) {
        ...
        setUnreadCount(0);
        if (typeof window !== "undefined") {
          localStorage.setItem("codestride:unread-notification-count", "0");
        }
          ...
          .catch(() => {
            ...
            if (typeof window !== "undefined") {
              localStorage.setItem(
                "codestride:unread-notification-count",
