...
export default function TopRepos() {
  ...
  const [bookmarkedRepos, setBookmarkedRepos] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const stored = window.localStorage.getItem("codestride_bookmarked_repos");
  ...
  useEffect(() => {
    const savedDays = localStorage.getItem("codestride_dashboard_range");
    if (savedDays) {
  ...
  useEffect(() => {
    localStorage.setItem("codestride_dashboard_range", String(days));
  }, [days]);

  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("codestride_bookmarked_repos", JSON.stringify(bookmarkedRepos));
    }
  }, [bookmarkedRepos]);
