...
export function useStreakTracker() {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      link.download = "codestride-streak.png";
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("codestride:sync", handleSync);
  }, [fetchStreak]);

  useEffect(() => {
    if (typeof window === "undefined" || !window.localStorage) return;
    const stored = window.localStorage.getItem(
      "codestride:dismissed-milestones"
    );

    const storedLastCelebrated = window.localStorage.getItem(
      "codestride:last-celebrated-milestone"
    );
