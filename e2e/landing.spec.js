...
test("[Landing E2E] page renders GitHub sign-in entrypoint", async ({ page }) => {
  ...
  ).toHaveAttribute("href", "https://github.com/omen18/codestride");
});
...
test("[Landing E2E] landing introduces CodeStride in an about section", async ({ page }) => {
  await page.goto("/");
...
test("[Landing E2E] landing shows footer", async ({ page }) => {
  ...
  await expect(page.getByText(/CodeStride. Built for open-source contributors/i)).toBeVisible();
});
