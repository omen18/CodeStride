# Caching in CodeStride

CodeStride uses a multi-layer caching strategy to reduce GitHub API pressure, improve dashboard load times, and stay within Vercel's serverless function budgets.
...
- Header: `x-codestride-cache-bypass: 1`
...
The leaderboard is expensive to build — it aggregates GitHub activity for every public CodeStride user. To avoid rebuilding it on every request, results are cached in the Supabase `leaderboard_cache` table.
...
All GitHub API calls in CodeStride use `cache: "no-store"` on the underlying `fetch`:
