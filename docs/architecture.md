# CodeStride Architecture

This page gives new contributors a map of how CodeStride's pages, API routes,
...
sequenceDiagram
  ...
  API->>DB: Resolve CodeStride user and linked GitHub accounts
...
CodeStride uses the Next.js App Router under `src/app`.
...
Route handlers live in `src/app/api`. CodeStride has 90+ routes across the following groups:
...
resolve the CodeStride user via `src/lib/resolve-user.ts`, then use the
...
| `resolve-user.ts` | Resolve NextAuth session to CodeStride Supabase user |
