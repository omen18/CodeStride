...
curl -X POST "https://<your-site>/api/leaderboard/rebuild" \
  -H "x-codestride-rebuild-token: $LEADERBOARD_REBUILD_TOKEN"
```
...
- Send concurrent requests to `/api/leaderboard` and verify `x-codestride-leaderboard-cache` header values (`supabase`, `stale-supabase`, etc.) and that only one rebuild occurs (check logs).
