...
across all CodeStride badge endpoints. It is intended for GSSoC contributors
...
CodeStride badge endpoints are **publicly accessible** — no authentication is
...
|---|---|---|
| **CodeStride badge rate limit** | `checkBadgeRateLimit()` in `src/lib/badge-rate-limit.ts` | 20 requests / minute / IP |
...
own rate limits on top of CodeStride's limits:
...
CodeStride badge endpoints use aggressive CDN caching to reduce both
CodeStride server load and GitHub API calls:
...
[![CodeStride Streak](https://codestride-silk-kappa.vercel.app/api/badge/streak-shield?user=YOUR_USERNAME)](https://codestride-silk-kappa.vercel.app/u/YOUR_USERNAME)
[![CodeStride Commits](https://codestride-silk-kappa.vercel.app/api/badge/commits?user=YOUR_USERNAME)](https://codestride-silk-kappa.vercel.app/u/YOUR_USERNAME)
```
...
- run: curl https://codestride-silk-kappa.vercel.app/api/badge/commits?user=me
...
If you run your own CodeStride instance, set `GITHUB_TOKEN` to increase
...
| HTTP 429 from CodeStride | >20 req/min from your IP | Wait for `Retry-After` seconds |
...
- [docs/caching.md](./caching.md) — Caching strategy across CodeStride
