# CodeStride Self-Hosting Guide

CodeStride is designed to be easily self-hostable. Since it uses Next.js and Supabase, you can deploy the web app anywhere Node.js or Docker runs, and you can point it to a free Supabase cloud database.
...
CodeStride uses `@supabase/supabase-js` which relies on the Supabase REST API, so a standard raw PostgreSQL container won't work out of the box. The easiest path is to use Supabase Cloud:
...
2. Set the **Homepage URL** to your deployment URL (e.g., `https://codestride.my-domain.com`).
...
| `NEXTAUTH_URL` | Yes | The canonical URL of your deployed app, no trailing slash | `https://codestride.my-domain.com` |
...
| `RESEND_FROM_EMAIL` | Optional | From address for outbound emails | `CodeStride <noreply@example.com>` |
...
CodeStride provides a production-ready `Dockerfile` and `docker-compose.yml`.

### Docker Variables Reference Manual

When running CodeStride with Docker, set the following environment variables in your `.env` file or container configuration.
...
1. Clone the repository and navigate into it:
   ```bash
   git clone https://github.com/Umbrella-io/codestride.git
   cd codestride
   ```
...
4. Start the container:
   ...
   docker compose up -d
   ```
5. CodeStride will be available at `http://localhost:3000`.

---

## 2. Railway

Railway is a modern PaaS that can deploy CodeStride directly from a GitHub repository.

[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/new/template?template=https%3A%2F%2Fgithub.com%2FUmbrella-io%2Fcodestride)

### Manual Setup
1. Click **New Project -> Deploy from GitHub repo** and select your fork of CodeStride.
...
CodeStride includes a `render.yaml` Blueprint for easy deployment on Render's free tier.

1. Create a free account on [Render](https://render.com/).
2. In the Render Dashboard, click **New + -> Blueprint**.
3. Connect your GitHub account and select your fork of CodeStride.
